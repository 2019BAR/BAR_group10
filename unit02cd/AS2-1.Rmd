...
```{r}
#
DD = as.POSIXct(Data$Date, format = '%m/%d/%y %H:%M')
#
...
```{r}
#
format(Data$Date, '%y') %>% table() %>% plot(,type = 'l')
#
```
...
```{r}
#
SDH = subset(Data, Date > median(Date))
sum(SDH$Arrest)
FDH = subset(Data, Date < median(Date))
sum(FDH$Arrest)
#
```
...
```{r}
#
table(format(Data$Date, '%y') , Data$Arrest) %>% prop.table(1) %>% round(2)
#
```
...
```{r}
#
table(Data$LocationDescription, exclude = 'OTHER') %>%sort()%>%tail(5)
#
```
...
```{r}
#
sum(table(Data$LocationDescription, exclude = 'OTHER') %>%sort()%>%tail(5))
#
```
...
```{r}
#
tapply(Data$Arrest, Data$LocationDescription, mean, na.rm = T)%>%which.max()
#
```
...
```{r}
#
tapply(Data$LocationDescription == 'GAS STATION', format(DD, '%w'), sum , na.rm = T)%>%which.max()
#
```
...
```{r}
#
tapply(Data$LocationDescription == 'DRIVEWAY - RESIDENTIAL', format(DD,'%w'), sum, na.rm = T)%>%which.min()
#
```
