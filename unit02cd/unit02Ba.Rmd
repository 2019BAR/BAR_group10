...
#mean of over15 by region
```


```{r}
tapply(WHO$LiteracyRate, WHO$Region, min, na.rm=TRUE)
#只要有一個是ＮＡ預設值就會是ＮＡ 打na.rm=TRUE就會先刪掉ＮＡ
```
...
```{r}
table(USDA$HighSodium) #高鹽種類食物的數量總和
```

```{r}
table(USDA$HighSodium, USDA$HighFat) #高鹽和高油種類食物數量的總和
```

```{r}
tapply(USDA$Iron, USDA$HighProtein, mean, na.rm=TRUE) #將含鐵食物根據高蛋白食物種類進行分組然後計算平均
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, max, na.rm=TRUE) # 將含維他命Ｃ食物根據高碳水化合物的食物種類進行分組然後找出其中最大值者
```

```{r}
tapply(USDA$VitaminC, USDA$HighCarbs, summary, na.rm=TRUE) #將含維他命Ｃ食物根據高碳水化合物種類食物進行分組然後算出敘述統計資料總覽
```
<br><br>
