...
```{r}
df[df$height > 180 , "name"] # 身高大於180的人的名字
```

```{r}
subset(df, height > 170 & !isMale)$name # 身高大於170且不是男生的人的名字
```

```{r}
mean(df$height[df$isMale]) # 所有男生身高平均
```

```{r}
df$height[!df$isMale] %>% mean # 所有女生的身高平均
```

```{r}
sum( subset(df, !isMale)$noBuy ) # 女生購買次數的加總
```

```{r}
subset(df, skin_color == "white" & !isMale ) %>% nrow # 白人女性的人數
```

```{r}
sum(df$skin_color == "white" & !df$isMale ) # 白種人女性總人數
```
