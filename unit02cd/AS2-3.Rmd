...
```{r}
#
table(DA$Married, DA$Region, exclude = NULL)
table(DA$Married, DA$Sex, exclude = NULL)
table(DA$Married, DA$Age, exclude = NULL)
table(DA$Married, DA$Citizenship, exclude = NULL)
#age影響大
```
...
```{r}
#
table(DA$State[is.na(DA$MetroAreaCode)]) %>% sort(decreasing = T)
#
```
...
```{r}
#
tapply(is.na(DA$MetroAreaCode), DA$State, mean) %>% sort()
...
```{r}
#
tapply(is.na(DA$MetroAreaCode), DA$Region, mean) %>% sort()
#Midwest
```
...
```{r}
#
table(DA$CountryOfBirthCode) %>% nrow
#
```
...
```{r}
#
A = merge(DA, CountryMap, by.x="CountryOfBirthCode", by.y="Code", all.x=TRUE)
#
```
...
```{r}
#
table(DA$MetroAreaCode) %>%　sort(decreasing = T)
#35620
```
...
```{r}
#
tapply(DA$Hispanic, DA$MetroAreaCode, mean) %>% sort(decreasing = T)
#29700
```
...
```{r}
#
tapply(DA$Race == 'Asian', DA$MetroAreaCode, mean) %>% sort(decreasing = T)
#26180,41860,41940,46700
```
...
```{r}
#
tapply(DA$Education == 'No high school diploma', DA$MetroAreaCode, mean, na.rm = T)
# 10420
```
...
```{r}
#
A = merge(A, MetroAreaMap, by.x ='MetroAreaCode', by.y = 'Code', all.x = T)
#
```
...
```{r}
#
table(A$Country) %>% sort(decreasing = T) %>% head(10)
#Philippines 
```
...
```{r}
#
table(A$MetroArea == 'New York-Northern New Jersey-Long Island, NY-NJ-PA', A$Country == 'United States') %>% prop.table(1) %>% round(3)
# 0.309
```
...
```{r}
#
tapply(A$Country == 'India', A$MetroArea, sum, na.rm = T) %>% sort(decreasing = T) %>% head(5)
#
```
...
```{r}
#
tapply(A$Country == 'Brazil', A$MetroArea, sum, na.rm = T) %>% sort(decreasing = T) %>% head(1)
#
```
...
```{r}
#
tapply(A$Country == 'Somalia', A$MetroArea, sum, na.rm = T) %>% sort(decreasing = T) %>% head(1)
#
```
